...
class TestSlackInteract:
    ...
    def test_slack_interact_unknown_payload_type(self, test_client, mock_app):
        """Test handling of unknown payload type returns 200."""
        app, mock_state = mock_app
        mock_state.slack_client.verify_slack_signature = lambda *args, **kwargs: True
    ...
    def test_slack_interact_approve_modal_open_fails(self, test_client, mock_app):
        """Test fallback when modal open fails."""
        app, mock_state = mock_app
        mock_state.slack_client.verify_slack_signature = lambda *args, **kwargs: True
        mock_state.slack_client._decode_request_data = lambda v: {
            "requester": "testuser",
        ...
        mock_state.slack_client.client = MagicMock()
        mock_state.slack_client.client.views_open = MagicMock(
        ...
        assert response.status_code == 200
        mock_state.slack_client.update_message_pending_approval.assert_called_once()

    def test_slack_interact_unknown_modal_callback(self, test_client, mock_app):
        """Test handling of unknown modal callback_id."""
        app, mock_state = mock_app
        mock_state.slack_client.verify_slack_signature = lambda *args, **kwargs: True
    ...
    def test_slack_interact_modal_submission_governance_error(self, test_client, mock_app):
        ...
        mock_state.governance_client.add_group_member = AsyncMock(
            side_effect=Exception("Governance API error")
        )
        ...
        assert "token_block" in body["errors"]
